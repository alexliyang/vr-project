...
from tools.yolo_utils import logistic_gradient, yolo_activate_regions
...
def YOLOLoss_np(y_true, y_pred, num_classes, priors, max_truth_boxes, thresh,
    ...
    for b in range(batch_size):
        for j in range(h):
            for i in range(w):
                for n in range(num_priors):
                    pred_box = yolo_get_region_box(y_pred[b, n * data_size:n * data_size + 4, j, i], n, i, j, w, h,
                                                   priors)
                    best_iou = 0.0
                    ...
                    if (best_iou > thresh):
                        ...
                        delta[b, n * data_size + 4, j, i] = noobject_scale * (
                            (0 - y_pred[b, n * data_size + 4, j, i]) * logistic_gradient(
                                y_pred[b, n * data_size + 4, j, i]))
        ...
        for t in range(max_truth_boxes):
            ...
            iou, delta = yolo_delta_region_box(delta, truth_box,
                                               y_pred[b, best_n * data_size:best_n * data_size + 4, j, i], best_n,
                                               best_index, i, j, w, h, priors, coord_scale)
            ...
            delta[best_index[0], best_index[1] + 4, best_index[2], best_index[3]] = object_scale * (
                iou - x) * logistic_gradient(x)

            truth_class = y_true[b, 0, t_j, t_i]
            # delta_region_class(best_index + 5, truth_class)
            for c in range(num_classes):
                if (c == truth_class):
                    delta[best_index[0], best_index[1] + 5 + c, best_index[2], best_index[3]] = class_scale * (
                        1 - y_pred[best_index[0], best_index[1] + 5 + c, best_index[2], best_index[3]])
                    avg_cat += y_pred[best_index[0], best_index[1] + 5 + c, best_index[2], best_index[3]]
                else:
                    delta[best_index[0], best_index[1] + 5 + c, best_index[2], best_index[3]] = class_scale * (
                        0 - y_pred[best_index[0], best_index[1] + 5 + c, best_index[2], best_index[3]])
            count += 1
            class_count += 1
...
def YOLOFscore(input_shape=(3, 640, 640), num_classes=45,
               priors=[[0.25, 0.25], [0.5, 0.5], [1.0, 1.0], [1.7, 1.7], [2.5, 2.5]], max_truth_boxes=30, thresh=0.6,
               nms_thresh=0.3):
